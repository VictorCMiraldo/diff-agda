...
module Diffing.Patches.Diff.Functor where
  ...
  mutual
    ...
    Dμ-map : ∀{a b}{n : ℕ}{t : T n}{ty : U (suc n)}
             {A : {n : ℕ} → T n → U n → Set a}
             {B : {n : ℕ} → T n → U n → Set b} 
           → ({m : ℕ}{t' : T m}{ty' : U m} → A t' ty' → B t' ty')
  ...
  --
  mutual
    D-map-join : ∀{a b c}{n : ℕ}{t : T n}{ty : U n}
                 {A : {n : ℕ} → T n → U n → Set a}
                 {B : {n : ℕ} → T n → U n → Set b}
                 {C : {n : ℕ} → T n → U n → Set c}
  ...
  mutual
    ...
    uncastμ : ∀{a}{n : ℕ}{t : T n}{ty : U (suc n)}
             {A : {n : ℕ} → T n → U n → Set a}
  ...
  D-map-cast : ∀{a b}{k : ℕ}{t : T k}{ty : U k}
                {A : {n : ℕ} → T n → U n → Set a}
                {B : {n : ℕ} → T n → U n → Set b}
                (f : {m : ℕ}{t' : T m}{ty' : U m} → A t' ty' → B t' ty')
                (d : Patch t ty)
  ...
  forget-cast : ∀{a}{n : ℕ}{t : T n}{ty : U n}
                {A : {n : ℕ} → T n → U n → Set a}
                (d : Patch t ty)
              → forget {A = A} (cast d) ≡ []
  forget-cast = trustme
  ...
  mutual
    ...
    Dμ-mult : ∀{a}{n : ℕ}{t : T n}{ty : U (suc n)}
             {A : {k : ℕ} → T k → U k → Set a}
